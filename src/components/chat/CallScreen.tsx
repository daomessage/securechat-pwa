...
// 每个通话状态对应的 UI 文案
//   hangup   = 主动挂断 / 收到对端 call_hangup
//   rejected = 对端拒接
//   ended    = SDK 兜底触发(disconnected 8s / RTP 静默 15s)— 网络断或对方崩了
//              文案区分让用户感知"自然挂"vs"网断/对方崩了"的差异
...
// 注: 终止态冻结 1.5s 退出由 App.tsx:201 的 onStateChange 处理
...
export function CallScreen() {
  ...
  // ── 通话计时器 ────────────────────────────────────────────────
  // - 'connected' 时累加 1s
  // - 终止态(hangup/rejected/ended) 时停止累加, durationSec 保留显示最终时长
  // - 'idle' / null / 新一轮通话 (calling/ringing) 时重置 0
  useEffect(() => {
    ...
    // 终止态: 不动 durationSec, 让 UI 显示通话结束前的最终时长
  }, [callState]);

  // 注: 1.5s 退出 timer 已在 App.tsx:201 的 onStateChange 处理里
  // (setTimeout(setCallState(null), 1500)),这里不再重复加 effect 避免冲突
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col select-none overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col h-full pointer-events-none">
        ...
        {isVideo && (callState === 'connected' || (isEnding && durationSec > 0)) ? (
          // 视频通话已接通 / 终止态(刚挂断): 紧凑顶栏 (名字 + 计时徽章)
          // 终止态保持紧凑顶栏 1.5s, 让用户看到"02:35 · 通话已结束"再退出
          <div className="flex items-center justify-between px-5 pt-3 pb-2 bg-gradient-to-b from-black/40 to-transparent">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                ...
                {isEnding ? (
                  <span className="font-mono text-red-400 text-xs drop-shadow">
                    {fmtDuration(durationSec)} · {STATE_LABELS[callState] ?? '通话已结束'}
                  </span>
                ) : (
          ...
          <div className="flex flex-col items-center pt-20 pb-6 px-6">
            ...
            <p className={`text-sm font-medium tracking-wide drop-shadow ${isEnding ? 'text-red-400' : 'text-blue-300'}`}>
              ...
              ) : isEnding && durationSec > 0 ? (
                // 终止态显示 "00:35 · 通话已结束" (有过通话时)
                <span>
                  <span className="font-mono">{fmtDuration(durationSec)}</span>
                  <span className="mx-1">·</span>
                  <span>{STATE_LABELS[callState] ?? callState}</span>
                </span>
              ) : (
