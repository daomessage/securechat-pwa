...
import { IOSPushHint } from '../pwa/IOSPushHint';
...
export function MainLayout() {
  ...
  return (
    <div className="flex flex-col vv-height bg-zinc-950 text-white pt-safe">
      <NetworkBanner />
      {/* 1.0.46: PWA 新版本提示 — 检测到 SW updatefound 后弹横幅引导用户刷新。
          iOS PWA 缓存激进, 不主动提示用户会长期跑旧版。 */}
      <UpdateBanner />
      {/* iOS PWA 通知授权引导 (App.tsx 自动 requestPermission 在 iOS 上必失败,
          需用户在设置里手动点 user-gesture 触发).
          注: 浏览器模式的安装引导由 InstallGate 全屏拦截(在 App.tsx 顶层渲染),
          不再用 InstallBanner 横幅(1.0.46 移除, 单一引导路径). */}
      <IOSPushHint />
