...
// nickname: 来自 friends 列表的 join,主标题用它(对齐 Android + 微信/iMessage 标准)
//           — alias_id 是机器标识,nickname 是用户起的可读名字,放主位用户认得出
type SessionWithPreview = SessionRecord & {
...
export function MessagesTab() {
  ...
  // 1.0.46: 删除原 showPwaBanner — 浏览器模式访问已被 InstallGate 全屏拦截,
  // 这个 banner 永远不会被触发到 (用户进不到 MessagesTab 就被 Gate 挡了)。
  ...
  const loadSessionsWithPreviews = async () => {
    ...
    // 同时拉 friends 列表用于 nickname join(对齐 Android 主标题用 nickname 设计)
    let friendsByAlias: Record<string, string> = {};
    try {
      const friends = await client.contacts.syncFriends();
      friendsByAlias = Object.fromEntries(
        friends.map((f: any) => [f.aliasId, f.nickname as string])
      );
    } catch {}
    ...
    const withPreviews = await Promise.all(rawSessions.map(async s => {
      ...
      return {
        ...
        nickname: friendsByAlias[s.theirAliasId] || undefined,
      };
    }));
      ...
      {sessions.length === 0 ? (
        ...
        <div className="flex flex-col">
          {sessions.map(s => {
            ...
            >
              <div className="w-12 h-12 bg-blue-500/20 rounded-full flex-shrink-0 flex items-center justify-center relative">
                {/* 对齐 Android: 头像用 nickname 首两位字母,fallback alias */}
                <span className="text-blue-400 font-bold">
                  {(s.nickname || s.theirAliasId).slice(0, 2).toUpperCase()}
                </span>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  {/* 主标题: nickname(对齐 Android + 微信/iMessage 标准),fallback @aliasId */}
                  <span className={`font-medium truncate ${unread > 0 ? 'text-white' : ''}`}>
                    {s.nickname || `@${s.theirAliasId}`}
                  </span>
                ...
                {/* 副标题: 最近消息预览,没消息时 fallback @aliasId 作为兜底标识 */}
                <div className={`text-sm truncate ${unread > 0 ? 'text-zinc-200 font-medium' : 'text-zinc-400'}`}>
                  {s.lastMessage ? renderPreview(s.lastMessage.text) : `@${s.theirAliasId}`}
                </div>
              </div>
            </div>
            );
          ...
          <div className="bg-zinc-900 rounded-2xl p-6 max-w-sm w-full shadow-2xl border border-zinc-700/50" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-400">与 {deleteTarget.nickname || `@${deleteTarget.theirAliasId}`} 的所有消息</p>
              </div>
            </div>
