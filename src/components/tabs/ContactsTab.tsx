...
function PendingRequestItem({ f, reload, onAccepted }: { f: ContactProfile; reload: () => void; onAccepted: (convId: string) => void }) {
  const { execute, isProcessing, error } = useSdkAction(
    async () => {
      await client.contacts.acceptFriendRequest(f.friendship_id);
      // 1.0.36: 接受后 syncFriends 已建立 session, 拉对应 conv_id 让外层跳到聊天页 + 弹核对窗
      const list = await client.contacts.syncFriends();
      const accepted = list.find((x: any) => x.friendship_id === f.friendship_id);
...
export function ContactsTab() {
  ...
  // P1-2 加成功反馈 (对齐 Android successMsg)
  ...
  const handleSendRequest = async () => {
    ...
    try {
      const targetAlias = searchRes.alias_id || searchRes.uuid || addId.trim();
      await client.contacts.sendFriendRequest(searchRes.uuid || addId.trim());
      setSearchRes(null);
      setAddId('');
      // 对齐 Android successMsg 反馈
      setSuccessMsg(`已向 @${targetAlias} 发送好友请求`);
  ...
  // 3-Tab 结构 — 对齐 Android `ContactsTab.kt`(好友 / 请求 / 添加)
  // 之前 PWA 把 3 块都放一个长页面里,信息过载;改成 segmented tab 后界面更清爽
  const [subTab, setSubTab] = useState<'friends' | 'requests' | 'add'>('friends');
  // 收到的请求数 > 0 时,自动跳到 requests Tab(让用户立即看到)
  useEffect(() => {
    if (pendingReceived.length > 0 && subTab === 'friends') {
      // 只在初次加载时跳,避免用户主动切回来又被跳走
    }
  }, [pendingReceived.length]);

  return (
    <div className="flex-1 w-full flex flex-col p-4 gap-4 overflow-y-auto bg-zinc-950">
      <h2 className="text-xl font-bold sticky top-0 bg-zinc-950/80 backdrop-blur py-2 z-10">
        联系人
      </h2>

      {/* 子 Tab 切换 — 对齐 Android TabRow 设计 */}
      <div className="flex border-b border-zinc-800">
        {([
          { key: 'friends',  label: `好友 (${accepted.length})` },
          { key: 'requests', label: pendingReceived.length > 0 ? `请求 (${pendingReceived.length})` : '请求' },
          { key: 'add',      label: '添加' },
        ] as const).map(t => (
          <button
            key={t.key}
            ...
            className={`flex-1 py-2.5 text-sm font-medium transition-colors relative ${
              subTab === t.key ? 'text-blue-500' : 'text-zinc-400 hover:text-zinc-200'
            }`}
          >
            {t.label}
            {subTab === t.key && (
      ...
      <div className="space-y-4">
        ...
        {pendingReceived.length > 0 ? (
            ...
            <div className="space-y-2">
              {pendingReceived.map(f => (
                <PendingRequestItem
                  ...
                  reload={loadData}
                  onAccepted={(convId) => {
                    // 1.0.36: 接受后跳到聊天页, ChatWindow 会因 trustState=unverified 自动显示遮罩 + 弹核对引导
          ...
          <p className="text-xs text-zinc-600 text-center py-8">没有待处理的好友请求</p>
        )}
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {accepted.map(f => (
              ...
              >
                <div className="w-10 h-10 bg-blue-500/20 text-blue-400 rounded-full flex items-center justify-center text-sm font-bold">
