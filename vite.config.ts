...
// 注入 SDK CHANGELOG.md 内容(Settings → 更新日志 用)
// 失败回退到空字符串, 确保 CF Pages 等不同环境都能跑
function safeReadChangelog(): string {
  try {
    // 优先读 node_modules 里的(npm 发布版),其次本地 monorepo
    const fromNode = resolve(__dirname, 'node_modules/@daomessage_sdk/sdk/CHANGELOG.md')
    return readFileSync(fromNode, 'utf8')
  } catch {
    try {
      return readFileSync(resolve(__dirname, '../sdk-typescript/CHANGELOG.md'), 'utf8')
    } catch {
      return '# Changelog\n\n暂无更新日志数据。'
    }
  }
}
const __CHANGELOG__ = safeReadChangelog()
...
export default defineConfig(({ mode }): UserConfig => ({
  ...
  define: {
    ...
    __CHANGELOG__: JSON.stringify(__CHANGELOG__),
  },
  plugins: [
    tailwindcss(),
    react(),
